...
class DateUtil {
  ...
  static DateTime stringToDateTime({required String date, bool isUTC = false, String? dateFormat}) {
    try {
      if (isUTC) {
        if (!date.contains("Z")) {
  ...
  static String getLeftTime(String stringDate) {
    ...
    if (elapsedDays < 1) {
      ...
      if (elapsedDays == 1) {
        ...
        stringResult = "${elapsedDays}d";
      } else if (elapsedWeeks >= 1 && elapsedMonths < 1) {
        stringResult = elapsedWeeks > 1 ? "$elapsedWeeks w" : "$elapsedWeeks w";
      } else if (elapsedMonths >= 1 && elapsedYears < 1) {
        stringResult = elapsedMonths > 1 ? "$elapsedMonths m" : "$elapsedMonths m";
      } else if (elapsedYears >= 1) {
        stringResult = elapsedYears > 1 ? "$elapsedYears y" : "$elapsedYears y";
      }
    } else {
  ...
  static String serverDateTimeFormat4 = "MMM dd, yyyy";
  ...
  static DateTime stringToDate(String string, {String format = "", bool isUTCtime = false, bool isReuireNullIfDateNotParse = false}) {
    ...
    if (format.isNotEmpty) {
      ...
      arrServerDateFormat.add(serverDateTimeFormat4);
    }
